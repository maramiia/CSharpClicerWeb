...
public class GetCurrentUserQueryHandler : IRequestHandler<GetCurrentUserQuery, UserDto>
{
    ...
    public async Task<UserDto> Handle(GetCurrentUserQuery request, CancellationToken cancellationToken)
    {
        ...
        var user = await appDbContext.ApplicationUsers
            ...
            .Include(user => user.UserArmors)
            .ThenInclude(ub => ub.Armor)
            .Include(user => user.UserWeapons)
            .ThenInclude(ub => ub.Weapon)
            .Include(user => user.UserBosses)
            .ThenInclude(ub => ub.Boss)
            .Include(user => user.UserBossImages)
        ...
        userDto.Protection = user.UserArmors.GetProfit();
        userDto.Power = user.UserWeapons.GetProfit();
        return userDto;
    }
}
